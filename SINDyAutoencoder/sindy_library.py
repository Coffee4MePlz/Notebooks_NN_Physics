...
class SINDyLibrary():
    ...
    def poly_deg_3(self, z):
        result = []
        for i in range(self.latent_dim):
            for j in range(i, self.latent_dim):
                for i2 in range(j, self.latent_dim):
                    res = z[:,i]*z[:,j]*z[:,i2]
                    res = res.reshape(-1, 1)
                    result.append(res)
        return torch.cat(result, axis=1)

    def poly_deg_4(self, z):
        result = []
        for i in range(self.latent_dim):
            for j in range(i, self.latent_dim):
                for i2 in range(j, self.latent_dim):
                    for j2 in range(i2, self.latent_dim):
                        res = z[:,i]*z[:,j]*z[:,i2]*z[:,j2]
                        res = res.reshape(-1, 1)
                        result.append(res)
        return torch.cat(result, axis=1)
    ...
    def fit(self):
        ...
        if self.poly_order > 1:
            ...
            if self.poly_order > 2:
                self.candidate_functions.append(self.poly_deg_3)
                names = []
                for i in range(self.latent_dim):
                    for j in range(i, self.latent_dim):
                        for i2 in range (j, self.latent_dim):
                            names.append(f'z{i}*z{j}*z{i2}')
                self.feature_names.extend(names)                
                if self.poly_order > 3:
                    self.candidate_functions.append(self.poly_deg_4)
                    names = []
                    for i in range(self.latent_dim):
                        for j in range(i, self.latent_dim):
                            for i2 in range (j, self.latent_dim):
                                for j2 in range(i2,self.latent_dim):
                                    names.append(f'z{i}*z{j}*z{i2}*z{j2}')
                    self.feature_names.extend(names)
